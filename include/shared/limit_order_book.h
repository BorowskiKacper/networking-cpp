...
namespace fh_lob
{
    ...
    public:
        // Pre-allocates every buffer the book will ever use; nothing here grows on
        // demand except price_level_maps/locate_to_str (see MapStockStr). All four
        // arguments are capacities, not objects -- note that the first two parameters
        // shadow the members they initialize.
        //
        // order_pool_capacity
        //     Max number of *live* orders at any instant. A slot is taken by AddOrder
        //     and returned by DeleteOrder (and by ReduceOrderSize once shares hit 0),
        //     so this only needs to cover peak concurrent open orders, not the day's
        //     total. Overflow throws std::runtime_error("pool exhausted") from
        //     MemoryPool::allocate_index.
        //     Cost: capacity * (sizeof(Order) == 20 B) + 4 B free-list entry.
        //
        // price_level_pool_capacity
        //     Max number of distinct (locate, price) pairs seen over the *entire
        //     session*. Levels are currently never recycled: the deallocate_index /
        //     map-erase in DeleteOrder is commented out, so an emptied level keeps its
        //     slot and its map entry forever. Size this for cumulative distinct prices
        //     across all symbols, not for concurrent levels.
        //     Cost: capacity * (sizeof(PriceLevel) == 16 B) + 4 B free-list entry.
        //
        // max_locates
        //     Expected number of ITCH stock locate codes; sizes price_level_maps and
        //     locate_to_str, both indexed directly by locate. MapStockStr doubles them
        //     (with a warning on stdout) if a larger locate arrives, but AddOrder and
        //     ReplaceOrder index price_level_maps[locate] with no bounds check -- so a
        //     locate must have been seen by MapStockStr (i.e. its Stock Directory
        //     message parsed) before any order message references it.
        //
        // max_order_ref_number
        //     One past the largest order reference number the feed will use. `orders`
        //     is a flat direct-lookup table of this size, indexed by order_ref_number
        //     with no bounds check, so anything >= this value is an out-of-bounds
        //     read/write. ITCH refs are 64-bit and increase monotonically through the
        //     day, so this is effectively "message volume for the session, rounded up".
